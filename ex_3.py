...
class MotDePasse:
    ...
    def __str__(self):
        ...
        return f"Mot de passe: {mdp_masquee}, Score: {score}/100"
class GenerateurMotdePasse:
    ...
    def generer_aleatoire(self, longueur=8, avec_symboles=False) -> MotDePasse:
        ...
        while True:
            mot = ''.join(random.choices(mdp_aleatoire, k=longueur))
            mdp = MotDePasse(mot)
            if (mdp.contient_minuscules()
                and mdp.contient_majuscules()
                and mdp.contient_chiffre()
                and (not avec_symboles or mdp.contient_symbole())
            ):
                return mdp
...
generateur = GenerateurMotdePasse().generer_aleatoire()